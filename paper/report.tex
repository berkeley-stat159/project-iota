...
\title{Working Memory Related Brain Network Connectivity in Individuals with 
...
\abstract{A recent study shows that schizophrenia reflects a ``dysconnection''
syndrome, which means schizophrenia can result in damaging functional brain 
connectivity in neural networks. We are interested in schizophrenia, so we 
decide to look into their studies to see if their conclusion makes sense. 
Therefore, we download the data they got during their tests and redo their 
analysis by following their steps in the paper. Even though we aren't able to 
go through all the analysis they did, we expect to get the same result for the 
analysis we do.}
